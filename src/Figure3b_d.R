...
# ===== CD4 cell proportions Responders vs Relapsed=====
...
data <- data[Response != "non-Responder"]


data$Response <- factor(data$Response, levels = c("Long-term Responder", "Relapsed"))
df2 <- data_summary(data, varname=c("Cellular_Proportion"), 
                    groupnames=c("Response","TimePoint_Final","Alias"))


data$TimePoint_Final <- factor(data$TimePoint_Final, levels = c("Leukapheresis","4 weeks after CAR T", "6 months after CAR T", "12 months after CAR T"))
...
df2$Response <- factor(df2$Response, levels = c("Long-term Responder", "Relapsed"))
...
data.plot <-  ggplot(data = df2, aes(x=TimePoint_Final, y=`Cellular_Proportion`, fill=Response)) + facet_wrap(~Alias, ncol=7, scales = "free_y")+
  ...
  geom_jitter(data = data, aes( x = TimePoint_Final, y =Cellular_Proportion,
                                fill =factor(Response, levels = c( "Long-term Responder","Relapsed"))),
...
# ===== CD4 cell proportions Responders vs non-Responder=====
setDT(total_proprotions_across_groups.1)
data <- total_proprotions_across_groups.1[regexpr("CD4.*|T regs",total_proprotions_across_groups.1$Alias)==TRUE]
data$Alias <- factor(data$Alias)
data <- data[Response != "Relapsed"]
data <- data[TimePoint_Final %in% c("Leukapheresis","4 weeks after CAR T")]


data$Response <- factor(data$Response, levels = c("Long-term Responder", "non-Responder"))
df2 <- data_summary(data, varname=c("Cellular_Proportion"), 
                    groupnames=c("Response","TimePoint_Final","Alias"))
...
df2$TimePoint_Final <- factor(df2$TimePoint_Final, levels = c("Leukapheresis","4 weeks after CAR T", "6 months after CAR T", "12 months after CAR T"))
df2$Response <- factor(df2$Response, levels = c("Long-term Responder", "non-Responder"))
...
data.plot <-  ggplot(data = df2, aes(x=TimePoint_Final, y=`Cellular_Proportion`, fill=Response)) + facet_wrap(~Alias, ncol=7, scales = "free_y")+
  ...
  geom_errorbar(aes(ymin=Cellular_Proportion-se, ymax=Cellular_Proportion+se), width=.2,
                position=position_dodge(.9)) +  
  geom_jitter(data = data, aes( x = TimePoint_Final, y =Cellular_Proportion,
                                fill =factor(Response, levels = c( "Long-term Responder","non-Responder"))),
              position = position_dodge(width = 0.9), stat = "identity",alpha=0.4, size=1) + 
  ggtitle("")+ ylab("Cellular Proportion (%)")+ xlab("") + 
...
# ===== CD8 cell proportions Responders vs non-Responder=====
setDT(total_proprotions_across_groups.1)
...
# ===== CD8 cell proportions Responders vs Relapsed=====
setDT(total_proprotions_across_groups.1)
data <- total_proprotions_across_groups.1[regexpr("CD8.*|NK T.*",total_proprotions_across_groups.1$Alias)==TRUE]
data$Alias <- factor(data$Alias)
data <- data[Response != "Relapsed"]
