...
metadata$Response <- factor(metadata$Response, c("non-Responder","Relapsed","Long-term Responder"))


######## Number of Expanded Clonotypes with TCR freq > 4
...
expanded_TCRs <- expanded_TCRs[Counts > 4]
...
data <- expanded_TCRs[regexpr("CD4.*|T regs",expanded_TCRs$Alias)==TRUE]
data$Alias <- factor(data$Alias)

data$Response <- factor(data$Response, levels = c("Long-term Responder", "Relapsed","non-Responder"))

df2 <- data_summary(data, varname="ExpandedTCRs",
                    groupnames=c("Response","TimePoint_Final","Alias"))

df2$Response <- factor(df2$Response, levels = c("Long-term Responder", "Relapsed","non-Responder"))
...
df2.subset <- df2[df2$Alias %in% c("CD4 CTL", "T regs"),]
data.subset <- data[data$Alias %in% c("CD4 CTL", "T regs"),]


df2.subset$TimePoint_Final <- factor(df2.subset$TimePoint_Final, levels = c("Leukapheresis","4 weeks after CAR T", "6 months after CAR T", "12 months after CAR T"))
df2.subset$Response <- factor(df2.subset$Response, levels = c("Long-term Responder", "Relapsed","non-Responder"))

cd4.plot <- ggplot(data = df2.subset, aes(x=TimePoint_Final, y=`ExpandedTCRs`, fill=Response)) + facet_wrap(~Alias, ncol=2, scales = "free_y")+
  ...
  geom_jitter(data = data.subset, aes( x = TimePoint_Final, y =ExpandedTCRs,
                                  fill =factor(Response, levels = c( "Long-term Responder","Relapsed","non-Responder"))),
...
expanded_TCRs <- setDT(expanded_TCRs)
data <- expanded_TCRs[regexpr("CD8.*|NK T .*",expanded_TCRs$Alias)==TRUE]
data$Alias <- factor(data$Alias)
...
df2.subset <- df2[df2$Alias %in% c("CD8 Tem/Highly cytotoxic", "CD8 Tem/Proinflammatory", "CD8 Tem/TGFb1+CXCR4+"),]
data.subset <- data[data$Alias %in% c("CD8 Tem/Highly cytotoxic", "CD8 Tem/Proinflammatory", "CD8 Tem/TGFb1+CXCR4+"),]
...
cd8.plot <- ggplot(data = df2.subset, aes(x=TimePoint_Final, y=`ExpandedTCRs`, fill=Response)) + facet_wrap(~Alias, ncol=2, scales = "free_y")+
  geom_bar(stat="identity", color="black",position=position_dodge(0.9), alpha=0.6, width=0.8) + scale_alpha_manual(values = c("0.5"=0.4, "1"=1), guide='none')+
  geom_errorbar(aes(ymin=ExpandedTCRs-se, ymax=ExpandedTCRs+se), width=.2,
                position=position_dodge(.9)) +  
  geom_jitter(data = data.subset, aes( x = TimePoint_Final, y =ExpandedTCRs,
