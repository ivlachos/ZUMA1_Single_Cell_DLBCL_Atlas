...
cytotoxic_activity_scores <- fread("/data/cytotoxic_activity_scores.txt")
...
metadata <- metadata[!(Patient_ID == "P10") & !TimePoint_Final == "12mo post-CAR T" & !TimePoint_Final == "6mo post-CAR T"]
...
df2$Condition <- factor(df2$Condition, levels = c("LtR","R","NR"))
...
ratio.plot <- ggplot(data = df2, aes(x=`Condition`, y=`CD8.CD4.ratio`, fill=Condition)) + facet_wrap(~`TimePoint_Final`, ncol = 2) +  
  geom_boxplot(data = df2,alpha=0.6, aes(x=`Condition`, y=`CD8.CD4.ratio`), width=0.3, outlier.size = FALSE,position=position_dodge(width = 0.9))+ ylab("CD8/CD4 T Cell ratio")+ xlab("")+ ggtitle("") + 
  ylim(0,10) + geom_jitter(data = df2, aes( x = Condition, y =CD8.CD4.ratio),position = position_dodge(width = 0.9), stat = "identity",alpha=0.4, size=0.5,fill="grey") + 
...
cytotoxic_activity_scores <- cytotoxic_activity_scores[TimePoint_Final %in% c("Leukapheresis","4 weeks after CAR T")]
cytotoxic_activity_scores$TimePoint_Final <- factor(cytotoxic_activity_scores$TimePoint_Final, levels=c("Leukapheresis","4 weeks after CAR T"))
