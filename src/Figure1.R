...
mycolors <- getPalette(length(unique(umap_embeddings$Compartment)))
...
myBreaks <- c(seq(min(dt), 0, length.out=3), 
              seq(1, max(dt), length.out=4))

figureC <- pheatmap_updated(mat = dt,cluster_rows=TRUE,cluster_cols =TRUE,fontsize = 7, 
                          fontsize_row = 7, fontsize_col = 7,annotation_colors=mycolors,
                          cellwidth=6.5, cellheight=9,color =inferno(10),name = "Log(Expr)",
                          breaks =myBreaks,
